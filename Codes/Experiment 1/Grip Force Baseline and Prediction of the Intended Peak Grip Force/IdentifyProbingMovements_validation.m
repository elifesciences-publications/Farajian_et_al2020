...
function ProbingIndex=IdentifyProbingMovements_validation(time,Py,Fy,SkinStr,Catch)

FlagIn=0;
ProbingIndex=[];
for i=1:length(Py)
    if (Py(i)<0 && FlagIn==0 &&Fy(i)>0)
...
for i=1:length(ProbingIndex)
    for j=ProbingIndex(i,1)+1:ProbingIndex(i,2)
        if Fy(j)==0
            ProbingIndex(i,2)=j;
            break;
        end
