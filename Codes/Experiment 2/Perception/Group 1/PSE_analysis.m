...
% - psychf.m
% - struct2batch.m
% - PsychometricFitting_PSE.m

% This file will produce Fig. 7(a) and 7(b) and the related statistical analysis.
%%
...
for d=1:FinalNumber
    ...
    if (d==1)
        forVector = zeros(360,1);
        forVector(1:160,1) = 21:180;
        forVector(161:320,1)= 201:360;
    else
        forVector = zeros(320,1);
        forVector(1:160,1) = 21:180;
        forVector(161:320,1)= 181:340; 
    end
...
% The function also returns the PSE values of the participant.
[Psych(1,d),Psych(2,d),Psych(3,d),Psych(4,d)]=PsychometricFitting_PSE(PsychometricData_SS0,PsychometricData_SS33,PsychometricData_SS66,PsychometricData_SS100);
end
%% Plotting
%% Fig. 7(b)
% The PSE values as a function of the tactor displacement gain
Gains = ([0 33 66 100])';
Psych_final = Psych;
...
colorcell = cell(1,10);
colorcell{1,1} = Colors(20,:);
colorcell{1,2} = Colors(24,:);
colorcell{1,3} = Colors(28,:);
colorcell{1,4} = Colors(32,:);
colorcell{1,5} = Colors(36,:);
colorcell{1,6} = Colors(40,:);
colorcell{1,7} = Colors(44,:);
colorcell{1,8} = Colors(48,:);
colorcell{1,9} = Colors(52,:);
colorcell{1,10} = Colors(56,:);
...
Yfit = b(1) + b(2)*[-10 33 66 110];
h2 = plot([-10 33 66 110],Yfit,'color',Colors(18,:),'LineWidth',4,'LineStyle',':');
ax = gca;
ax.FontSize = 12;
...
Psych_subjects = reshape(Psych,[40,1]);
...
subject_Num = [ones(4,1);2*ones(4,1);3*ones(4,1);4*ones(4,1);5*ones(4,1);6*ones(4,1);,...
    7*ones(4,1);8*ones(4,1);9*ones(4,1);10*ones(4,1)];
